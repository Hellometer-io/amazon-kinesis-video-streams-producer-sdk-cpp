...
static gboolean retry_pipeline(GstElement *pipeline) {
    GstStateChangeReturn ret;

    ret = gst_element_set_state(pipeline, GST_STATE_NULL);
    if (ret == GST_STATE_CHANGE_FAILURE) {
        g_printerr("Unable to set pipeline to NULL state.\n");
...
static gboolean bus_callback(GstBus *bus, GstMessage *msg, gpointer data){
    ...
    case GST_MESSAGE_EOS:
      ...
      g_timeout_add_seconds(5, (GSourceFunc)retry_pipeline, data);
      g_print("Restarting pipeline in 5 seconds...\n");
      break;
    case GST_MESSAGE_ERROR: {
      ...
      /* Restart pipeline after 5 seconds */
      g_timeout_add_seconds(5, (GSourceFunc)retry_pipeline, data);
      g_print("Restarting pipeline in 5 seconds...\n");
      break;
    }
